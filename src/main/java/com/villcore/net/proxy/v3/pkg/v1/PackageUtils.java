...
//    public static void xrelease(Package pkg) {
...
//    }
//
////    public static void xrelease(ByteBuf byteBuf) {
////        if (byteBuf != null) {
////            release2(byteBuf);
////        }
////    }
//
//    public static void xrelease2(Package pkg) {
////        ByteBuf fix = pkg.getFixed();
////        ByteBuf header = pkg.getHeader();
////        ByteBuf body = pkg.getBody();
////        release2(fix);
...
//    }
//
//    public static void xrelease2(ByteBuf byteBuf) {
//
...
//    }
    ...
    public static void release(ByteBuf byteBuf) {
        LOG.debug("byte buf ref = {}", byteBuf.refCnt());
            if (byteBuf instanceof CompositeByteBuf) {
                CompositeByteBuf.class.cast(byteBuf).iterator().forEachRemaining(b -> {
                    if (b.refCnt() > 0) {
                        b.release(b.refCnt());
                    }
                });
    ...
    public static void release(Optional<Package> pkg) {
        pkg.map(p -> p.getClass().cast(p)).ifPresent(p -> {
            ByteBuf byteBuf = p.toByteBuf();
...
//
//                case PackageType.PKG_CONNECT_REQ:
//                    printRef("cconnect pkg ref", p);
//                    LOG.debug("to byte buf ref = {}", p.toByteBuf().refCnt());
//                    p.getFixed().release();
//                    break;
//
//                case PackageType.PKG_CONNECT_RESP:
